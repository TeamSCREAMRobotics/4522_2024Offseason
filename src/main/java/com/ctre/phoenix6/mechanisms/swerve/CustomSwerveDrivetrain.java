 ...
 */
public class CustomSwerveDrivetrain {
  ...
  public class OdometryThread {
    ...
    public void run() {
      ...
      while (m_running) {
        ...
        StatusCode status;
        if (IsOnCANFD && !Utils.isSimulation()) {
  ...
  public CustomSwerveDrivetrain(
    ...
    if (driveTrainConstants.Pigeon2Configs != null) {
      StatusCode response = m_pigeon2.getConfigurator().apply(driveTrainConstants.Pigeon2Configs);
      if (!response.isOK()) {
        System.out.println(
            "Pigeon2 ID "
