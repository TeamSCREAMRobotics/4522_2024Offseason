...
public class ShootSimNote extends Command {
    ...
    public void initialize() {
        ...
        duration = calculatedTrajectory[0].getDistance(calculatedTrajectory[calculatedTrajectory.length - 1]) / Conversions.rpsToMPS(shooterVelocity, ShooterConstants.WHEEL_CIRCUMFERENCE.getMeters(), 1.0);
        shooterAngle = subsystems.pivot().getAngle().getRadians();
        robotAngle = subsystems.drivetrain().getHeading().getRadians();

        RobotContainer.getRobotState().addActiveNote(() -> new Pose3d(getNoteTranslation(), new Rotation3d(0, shooterAngle, robotAngle)));
    }
